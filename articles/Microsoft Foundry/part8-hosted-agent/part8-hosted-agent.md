---
title: "Build, deploy, and operate a hosted code agent"
...
articleId: 79b6b4b3-d395-4114-a062-ae3683830555
...
publishAt: "2026-08-02T13:00:00.000Z"
---
# Hosted Agents
...
Copy-Item ..\.env.example code\.env
```
...
## Foundry hosted agents versus Azure Container Apps

The Python code is not locked to Foundry Agent Service. A Microsoft Agent Framework agent is still a Python application, so you can package the same application in a Docker image and run it in **Azure Container Apps** instead. Container Apps can expose the agent through your own HTTP API, while Foundry hosted agents expose it through the hosted-agent contract and make it available in the Foundry experience.

### Why choose a Foundry hosted agent?

Foundry hosted agents are valuable when you want to operate an application as an **agent**, not merely as a container that happens to call a model:

- **Agent-native discoverability:** The deployed agent appears in the Foundry project under **Build > Agents**, where developers and stakeholders can find the agent, select its active version, and test it without first learning a separate service URL or custom client.
- **A standard agent endpoint:** Foundry provides the hosted-agent invocation contract, such as `/responses`. Consumers can use a consistent agent-facing protocol instead of every team inventing a different HTTP API, request schema, streaming behavior, and error format.
- **A managed agent lifecycle:** Foundry gives the deployment an agent identity and versioned activation model. Teams can promote a tested version, keep older versions available for rollback or comparison, and separate the agent definition from the deployment mechanism.
- **Less platform glue:** Your team focuses on the agent instructions, tools, orchestration, and business behavior. Foundry supplies the surrounding project integration, endpoint exposure, hosted runtime, identity wiring, and operational entry point that you would otherwise assemble around a Container App.
- **One place for the AI estate:** Prompt agents, hosted agents, models, evaluations, traces, and project configuration can be managed from the same Foundry boundary. This makes it easier to govern an organization-wide collection of agents than a set of unrelated containerized APIs.
- **A natural path from prototype to custom code:** A team can start with a prompt agent, move custom tools or Microsoft Agent Framework orchestration into a hosted agent, and keep the agent in the same Foundry-centered operating model.

These benefits matter most when an organization has multiple agents, multiple teams, or a requirement for a consistent agent platform. They are less compelling for a small internal API where the surrounding application already owns the endpoint, deployment pipeline, networking, and monitoring.

That means the main difference is not whether the code can run. Both platforms can run the container. The difference is which platform owns the agent-facing runtime and operational experience:

| Concern | Foundry hosted agent | Azure Container Apps |
| --- | --- | --- |
| Agent registration | Appears as a versioned agent under **Build > Agents** | You expose and manage your own application endpoint |
| Invocation contract | Foundry hosted-agent protocol, such as `/responses` | API contract that you design and maintain |
| Agent lifecycle | Foundry Agent Service manages hosted-agent versions and activation | You manage Container Apps revisions, traffic, and deployment |
| Application hosting | Foundry supplies the hosted-agent runtime around your code | Container Apps supplies general-purpose container hosting |
| Best fit | An agent that should be discoverable, testable, and operated as a Foundry agent | An agent that is part of a broader application or needs platform-level container control |
| Trade-off | More opinionated runtime and tighter Foundry integration | More control, but you own the API surface and more of the surrounding application plumbing |

Choose a Foundry hosted agent when the agent itself is the product boundary: you want it registered in Foundry, tested in the Foundry experience, invoked through a standard agent protocol, and operated alongside Foundry projects, models, evaluations, and agent versions. Choose Container Apps when the agent is one component inside a wider service: for example, an API, worker, web application, or event-driven system that needs custom routing, networking, scaling rules, or multiple containers.

The same Agent Framework orchestration can therefore be deployed either way. Moving from Foundry hosted agents to Container Apps does not remove the need for application-level authorization, validation, approvals, retries, timeouts, and observability; it generally increases the amount of that platform and API responsibility your team must implement.

This article focuses on Foundry hosted agents because the goal is to show how a Python application becomes a discoverable, versioned, testable agent in Foundry—not just a container with an HTTP endpoint. Container Apps remains a valid hosting option, but choosing it means giving up that agent-native Foundry experience and owning more of the surrounding service contract yourself.
...
Next: [Part 9 - Manual orchestration with prompt agents](/blog/microsoft-foundry/part9-multi-agent-orchestration), [Part 10 - Agent Framework orchestration](/blog/microsoft-foundry/part10-agent-framework-orchestration), [Part 11 - Agent2Agent communication](/blog/microsoft-foundry/part11-agent-to-agent-communication), [Part 12 - From notebook to production](/blog/microsoft-foundry/part12-from-notebook-to-production), and [Part 13 - Agent Insights](/blog/microsoft-foundry/part13-agent-insights).
