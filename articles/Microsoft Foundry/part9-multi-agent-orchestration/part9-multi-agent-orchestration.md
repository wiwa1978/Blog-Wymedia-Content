---
title: "Manual Multi-Agent Orchestration with Prompt Agents"
excerpt: "Learn the mechanics of routing, sequential orchestration, and parallel reviews by manually coordinating Microsoft Foundry prompt agents."
slug: microsoft-foundry/part9-multi-agent-orchestration
articleId: 1dfbf811-b1d4-4eef-a314-0532105747c2
...
publishAt: "2026-08-06T13:10:00.000Z"
---
# Microsoft Foundry SDK: Part 9 - Manual Multi-Agent Orchestration with Prompt Agents
...
This article builds that progression with Microsoft Foundry prompt agents and explains the orchestration concepts behind Microsoft Agent Framework. It is intentionally a **manual, conceptual implementation**: the application owns routing, sequencing, and fan-out so you can see the mechanics before adopting a workflow framework.
...
> **Important:** The executable samples in this article use **manual application-level orchestration**. They deliberately call prompt agents through the Responses API, parse the handoff result, pass text between agents, and use Python threads for fan-out. This makes the mechanics visible, but it is not the production-recommended implementation. For real multi-agent workflows, use the Microsoft Agent Framework builders shown in [Part 10](../part10-agent-framework-orchestration/part10-agent-framework-orchestration.md).

## Understand the orchestration patterns

This article does not use Microsoft Agent Framework. It uses the Azure AI Projects Responses API and ordinary Python code so that the mechanics remain visible: the application classifies the request, dispatches to a specialist, passes text between stages, and starts parallel calls.

The same conceptual patterns can later be implemented by an orchestration framework. Part 10 shows the production-oriented version with Microsoft Agent Framework, while Microsoft Foundry supplies the project, model deployments, hosting, identity, tracing, evaluation, and operational controls around that workflow.

The five patterns to recognize are:
...
Copy the shared `../.env.example` to `code/.env`, then fill in `AZURE_AI_PROJECT_ENDPOINT`, `MODEL_DEPLOYMENT`, and a unique `AGENT_PREFIX`.
...
## Step 3: Add manual application-level handoff routing
...
With the default routing prompt, or with the part 9 `TEST_PROMPT` uncommented in the shared `.env.example`, the output is similar to:
...
## Step 4: Manual sequential orchestration

Some tasks have a dependency: one specialist must produce a draft before another specialist can inspect it. Here, the shopper specialist receives the jacket recommendation request and writes the first draft. **The application then takes the shopper's response text and inserts it into a new prompt for the reviewer specialist.** The reviewer checks that embedded draft for unsupported claims and missing assumptions, then writes the revised answer. The reviewer is not answering the original request independently; it is reviewing the shopper specialist's output.
...
The important detail is how the second agent receives the first agent's work. The reviewer does not automatically know what the shopper specialist said. **There is no shared conversation or automatic framework handoff in this Part 9 example.** The application takes the draft, adds instructions such as “check the assumptions and unsupported claims,” and sends that complete text as a new request:

```python
review_prompt = (
    "Review this draft for unsupported claims and missing assumptions. "
    "Return a corrected, concise version.\n\nDRAFT:\n"
    # The shopper response becomes the reviewer's input.
    + response_text(draft)
)
review = run_agent(project_client, AGENT_NAMES["reviewer"], review_prompt)
```

The `response_text(draft)` expression is the handoff: it converts the shopper response to text, and string concatenation places that text after `DRAFT:` in `review_prompt`. The subsequent `run_agent(..., AGENT_NAMES["reviewer"], review_prompt)` call sends the combined prompt to the reviewer. This makes the workflow easy to understand: the first agent writes, the application explicitly passes its text to the second agent, and the second agent reviews it. In a larger application, you can add other information to that second request, such as a product ID or customer request, so the reviewer has the context it needs.
...
## Step 5: Manual concurrent orchestration
...
This is called **concurrent orchestration**. In this manual version, the application sends the request to all three specialists, waits for their answers, and then collects the results. The agents do not automatically see one another's answers, and there is no routing decision that picks only one of them. This is useful for explaining the pattern; Part 10 uses `ConcurrentBuilder` for the production-oriented version.
...
- These manual examples explain the mechanics; Part 10 implements the same patterns with Microsoft Agent Framework.

Next up: **Part 10** - Orchestrate Foundry-backed agents with Microsoft Agent Framework.

---

*Sources: [Build with agents, conversations, and responses](https://learn.microsoft.com/azure/ai-foundry/how-to/develop/sdk-overview), [Microsoft Agent Framework](https://learn.microsoft.com/azure/foundry/agents/how-to/agent-framework).*
